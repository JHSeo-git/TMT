...
| Warm query | 14–27ms against the real archive, by term | Once built, the route is not the bottleneck |
| Cold query, end to end | 1.2s with the entries baked at build time | Assembling them per process ran past Vercel's 10s limit |
...
`app/api/search/route.ts` delegates to the `GET` handler `createSearchAPI` returns, and the entries
it indexes are built ahead of time by `scripts/generate-search-index.ts`, which runs before
`next build`.

That split is not tidying. Assembling the entries inside the route meant three GitHub requests and
`structure()` over 293 bodies on every cold start: 4.8s before Orama began locally, and past
Vercel's 10s function limit in production, where the route answered
`FUNCTION_INVOCATION_TIMEOUT`. It was measured locally and never against the deployment, which is
how it shipped. Everything up to Orama depends only on the deployment's content, so it belongs at
build time; a cold instance now pays for Orama's index and nothing else, and answers its first
query in 1.2s. Warm queries are 14ms.

The generated file is 4.78MB of JSON, 14,208 chunks over 293 items, gitignored and regenerated by
the `build` script. Turbopack inlines it as `JSON.parse('…')` rather than an object literal, which
is the fast path. `dev` generates it only when it is missing, so restarting the dev server does not
spend four seconds on GitHub each time; `bun run search-index` forces a refresh.

Runtime caching was considered and is still not used. `unstable_cache` has a per-entry size limit
that 2.4MB of bodies would run into, and Next 16's `"use cache"` needs the top-level
`cacheComponents` flag, which changes caching and prerendering for the whole app. Neither is needed
now that the expensive half happens at build time — which suits a site whose content only changes
on redeploy anyway.
