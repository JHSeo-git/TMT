...
- 스킬 검증으로 실제 아이템 네 건이 분류됐습니다: #155 「React 19.2」와 #230 「현대 브라우저는 어떻게 동작하는가?」에 `topic/frontend`, #297 「어떤 규모에서도 동작하는 Git」에 `topic/platform`, #302 `dump`에 `secondthought/skipped`.
...
- 스킬 문서와 CLI를 영어로 작성했습니다(`SKILL.md`, `scripts/queue.ts`의 주석·메시지·출력). 스킬은 에이전트가 읽는 지침이라 영어가 기본값에 가깝고, 나중에 GitHub Action 로그로 나갈 출력도 영어가 낫습니다. 다만 실제 이슈 제목 인용과 `CONTEXT.md`가 정한 용어(주제/카테고리)는 한국어로 남겼는데, 번역하면 가리키는 문자열이 실제와 달라져 오히려 틀리기 때문입니다. `lib/labels.ts`와 `docs/` 아래 문서는 저장소의 기존 관행대로 한국어를 유지합니다.
